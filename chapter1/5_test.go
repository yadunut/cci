...
func TestBetterBetterCompressString(t *testing.T) {
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := BetterBetterCompressString(tt.args.s); got != tt.want {
				t.Errorf("BetterCompress() = %v, want %v", got, tt.want)
			}
		})
	}
...
func BenchmarkBetterBetterCompressString(b *testing.B) {
	for i := 0; i < b.N; i++ {
		BetterBetterCompressString("aaaabbbbccccddddeeee")
	}
}
